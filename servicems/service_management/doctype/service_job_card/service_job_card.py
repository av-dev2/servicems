...
class ServiceJobCard(WebsiteGenerator):
	...
	def update_tables(self):
		# Appends to a different child table than it iterates, which is safe.
		for template in self.services:  # nosemgrep
	...
	@frappe.whitelist()
	def create_parts_entry(self, type: str):
		if not self.parts:
		...
		for item_row in self.parts:
			if item_row.service_parts_entry or item_row.use_existing_spares or not item_row.qty:
				continue
	...
	def update_supplied_parts_details(self, supplied_items, parts_entry_no):
		# Appends to a different child table than it iterates, which is safe.
		for row in self.parts:  # nosemgrep
	...
	@frappe.whitelist()
	def create_stock_entry(self, type: str):
	...
	def get_price_list(self, template_price_list=None):
		...
		if not price_list:
			price_list = frappe.get_value("Service Settings", "Service Settings", "price_list")
...
@frappe.whitelist()
def get_selected_items(items: str | list):
	...
	if selected_items:
		...
		for item in selected_items:
			if not item.get("qty_to_return"):
				frappe.throw(
					_(
						'<h4 class="text-center" style="background-color: yellow; font-weight: bold;">Can not process stock entry for empty quantity to return</h4>'
					)
				)
		...
		if new_doc.get("name"):
			...
			frappe.msgprint(_("Stock Entry: {0} Created Successfully").format(frappe.bold(new_doc.name)))
		else:
			frappe.throw(_("Stock Entry was not created, try again"))
...
@frappe.whitelist()
def get_all_supplied_parts(job_card: str):
