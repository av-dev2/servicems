  ...
  <div class="scroll h-[600px] overflow-y-auto">
    ...
    <div v-for="(bookings, date) in weeklyBookings" :key="date" class="mb-5">
      <div class="border border-gray-300 rounded-md p-4 relative">
        <h1
          class="flex shadow-lg shadow-green-500 font-bold mb-8 text-center h-7 text-2xl"
        >
          {{ date }}
        </h1>
        ...
        <div class="flex items-center justify-between relative">
          ...
          <div
            :id="`container-${date}`"
            class="flex overflow-x-hidden gap-4 px-8 h-40 custom-scroll-hidden relative"
          >
            <div
              ...
              :class="[
                !item.status
                  ? 'bg-white-overlay-500 border-0 shadow-xl shadow-cyan-500/50 font-sans'
                  : 'bg-blue-300 border-blue-300 shadow-lg shadow-cyan-500/50 font-mono',
              ]"
              class="w-[140px] h-[150px] flex-shrink-0 rounded-md shadow-md p-2 flex flex-col justify-between"
            >
              <div
                class="flex-1 overflow-hidden text-ellipsis whitespace-nowrap mt-2 text-center"
              >
                ...
                >
                  Status:
                  {{ item.status === 'In Progress' ? 'Progress' : item.status }}
                </p>
              </div>
...
const weeklyBookings = computed(() => {
  ...
  lastcount.value = filters.to_date
    ? differenceInDays(new Date(filters.to_date), today.value)
    : 6
  ...
  const allBays = bayDataList
    .filter((item) => item.bay_name)
    .map((item) => ({
    ...
    bayDataList.forEach((item) => {
      ...
      if (itemDate) {
        ...
        if (dates[formattedItemDate]) {
          const bayIndex = dates[formattedItemDate].findIndex(
            (bay) => bay.bay_name === item.bay_name
          )
          if (bayIndex !== -1) {
...
function openBookingDoc(bay_name, date) {
  ...
  window.location.href =
    '/app/service-booking?bay=' + bay_name + '&booking_date=' + bDate
}
